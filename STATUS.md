...
- The README and both CI/release workflow examples used mutable Go `1.26.x` while the publisher gate used checksum-reviewed Go `1.26.5`. All three public paths now match the reviewed patch exactly.
- Publisher contract coverage rejects the mutable patch selector in README and workflows; runtime behavior and release assets are unchanged, and aggregate trial remains non-adoption evidence.
...
- Updated the copy-ready Action example and its publisher regression to the current immutable public main revision; the published release and runtime behavior remain unchanged.
- Aggregate trial remains weak and no verified external use is claimed.
...
- `2026-07-21T13:39:23Z`: 全3 managed repositoryをbrokerで再確認し、main CI success、open Issue／PR 0、各v0.1.0 releaseの4 archive＋`SHA256SUMS`を確認した。対象projectはview 1／unique view 1／referrer visit 1、clone 0、release download 8のままで、外部repository参照は検索で確認できず、downloadはowner／repair由来を除外できないためverified external useは0とした。Dependency `go.yaml.in/yaml/v3@v3.0.4`はdeps.dev advisory 0、OSV vulnerability 0を維持する。Exact-matchのopen・unlocked `chartmuseum/helm-push#243`へverification bypassを常態化しない診断を先に示す1回限りのcommunity commentをbrokerへ依頼したが、GitHub API 403で拒否され、source of truthのcomment count 0を再確認した。迂回せず、project health=`healthy`、decision=`monitor`、24時間review時刻を維持する。
- `2026-07-21T17:09:39Z`: 未反映engagementがないことを確認し、全3 managed repositoryをmetrics／status brokerで検査した。全repositoryのcurrent main CIはsuccess、open Issue／PRは0、各v0.1.0 releaseは4 archive＋`SHA256SUMS`の5 assetを保持する。対象projectはview 1／unique view 1／referrer visit 1、clone 0、release download 8で変化せず、downloadをowner／repair由来から分離できないためverified external useへ数えない。Dependency `go.yaml.in/yaml/v3@v3.0.4`はdeps.devでMIT／Apache-2.0・advisory 0、OSV vulnerability 0を維持し、deps.dev project indexは未収載だった。直近のbounded third-party Issue searchにもexact matchはなく、health=`healthy`、decision=`monitor`、24時間review時刻を維持する。
- `2026-07-23T12:35:52Z`: 全6 managed repositoryをcredential-isolated status／metrics brokerで同期し、current main CI success、open Issue／PR 0、各latest releaseの4 archive＋`SHA256SUMS`を確認した。対象projectは14日windowでviews 7（unique 1）、clones 94（unique 48）、release downloads 8だが、個票がなく独立利用者へ帰属できないためtrial／weakを維持する。一般的な discovery command `helm4-plugin-preflight --help` がunknown command／exit 2になるfunnel defectを再現し、`--help`、`-h`、`help`でtop-level usageをstdoutへ出してexit 0にするcontract、並列unit test、README、publisher gateを追加した。
- `2026-07-28T07:15:20Z`: 全6 managed repositoryのbroker status／metricsを同期し、current main CI success、open Issue／PR 0、latest release asset欠損0を確認した。対象projectは14日windowでviews 12（unique 1）、clones 110（unique 58）、release downloads 8だが、個票がなく独立利用者へ帰属できないためtrial／weakを維持する。公開releaseが`v0.1.1`、public mainが`ff874902ac4ba4a84ab17749dcfa5779ec5bf297`へ進んだ後もREADMEのinstall／archive／Action／packaging例が`v0.1.0`のままというfunnel defectを修正し、publisher gateへrelease URL、archive名、go install、immutable Action SHAの整合検査を追加した。
